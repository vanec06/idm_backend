...
export const listarMaquinas = async (req, res) => {
    try {
        ...
        if (req.body.limite == true) {
            ...
            if (req.body.area) {
                const posicion = condiciones.findIndex(condicion => condicion.includes('area.id_area ='))
                if (posicion != -1) {
                    condiciones.splice(posicion, 1);
                }
            ...
            if (req.body.ambiente) {
                const posicion = condiciones.findIndex(condicion => condicion.includes('ambiente.id_ambiente = '))
                if (posicion != -1) {
                    condiciones.splice(posicion, 1);
                }
                if (req.body.ambiente != 'null') {
                    condiciones.push('ambiente.id_ambiente = ' + req.body.ambiente);
                }
            }
            if (req.body.buscar) {
                const posicion = condiciones.findIndex(condicion => condicion.includes('maquina.nombre LIKE '))
                if (posicion != -1) {
                    condiciones.splice(posicion, 1);
                }
