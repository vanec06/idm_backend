...
export const listarAmbientes = async (req, res) => {
    try {
        const [result] = await pool.query('select area.nombre as nombre_area, ambiente.*, CONCAT(usuario.nombres, " ", usuario.apellidos) as encargado from ambiente join area on ambiente.area_id_area = area.id_area join usuario on ambiente.id_usuario = usuario.id_usuario');
